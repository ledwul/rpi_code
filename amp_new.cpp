  ...
  while (1) {
    ...
    for (int light = 0; light < 4; light++) {
      ...
      if (target < 0.01) {
        ...
        if (val[light] > steps + maxdelta)
          val[light] = steps + maxdelta;
        if (val[light] < -maxdelta)
          val[light] = -maxdelta;
      }
      ...
      if (index % 1000 == 0) {
        cout << "Light " << light << ": " << cur << "A (" << val[light] << "), "
             ...
             << "V, Target: " << getTarget(light, time) << "A." << endl;
      }
    }
  }
  return 0;
...
float getTarget(int light, float time) {
  ...
  float ramp_time;
  // Use different Ramp Times for different LEDs such that we start with the warmer colors
  if (light % 2 == 0) {
    ramp_time = 10;
  } else {
    ramp_time = 20;
